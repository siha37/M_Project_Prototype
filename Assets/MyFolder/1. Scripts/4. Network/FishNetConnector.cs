...
namespace MyFolder._1._Scripts._4._Network
{
    public class FishNetConnector : MonoBehaviour
    {
        ...
        void InitializeFishNet()
        {
            ...
            // UnityTransport 찾기 (Relay 경로용)
            if (unityTransport == null && fishNetworkManager != null)
            {
                unityTransport = fishNetworkManager.GetComponent<UnityTransport>();
                if (unityTransport == null)
                {
                    unityTransport = fishNetworkManager.gameObject.GetComponent<UnityTransport>();
                }
            }
        
            if (fishNetworkManager == null)
            {
                LogManager.LogError(LogCategory.Network, "FishNet NetworkManager를 찾을 수 없습니다!");
                return;
            }
        ...
        // Relay 호스트 시작(최소 기능)
        public async System.Threading.Tasks.Task<(bool ok, string joinCode)> StartHostRelayAsync(int maxPlayers = 7)
        {
            try
            {
                UpdateFishNetStatus("Relay 호스트 시작 중...");
                ...
                if (fishNetworkManager == null)
                {
                    UpdateFishNetStatus("NetworkManager 없음");
                    return (false, null);
                }

                var (alloc, code) = await RelayConnector.CreateHostAsync(maxPlayers);
                ...
                if (unityTransport == null)
                {
                    UpdateFishNetStatus("UnityTransport 없음");
                    return (false, null);
                }
                ...
                UpdateFishNetStatus("Relay 호스트 시작 완료");
                return (true, code);
            }
            catch (System.Exception ex)
            {
                UpdateFishNetStatus($"Relay 호스트 시작 오류: {ex.Message}");
                OnFishNetError?.Invoke(ex.Message);
                return (false, null);
            }
        }

        // Relay 클라이언트 접속(최소 기능)
        public async System.Threading.Tasks.Task<bool> ConnectClientRelayAsync(string joinCode)
        {
            try
            {
                UpdateFishNetStatus($"Relay 클라이언트 연결 중... code={joinCode}");
                ...
                var join = await RelayConnector.JoinAsync(joinCode);
                ...
                unityTransport.SetRelayServerData(AllocationUtils.ToRelayServerData(join, "dtls"));

                fishNetworkManager.ClientManager.StartConnection();
                isFishNetConnected = true;
                OnFishNetConnectionChanged?.Invoke(true);
                UpdateFishNetStatus("Relay 클라이언트 연결 성공");
                return true;
            }
            catch (System.Exception ex)
            {
                UpdateFishNetStatus($"Relay 클라이언트 연결 오류: {ex.Message}");
                OnFishNetError?.Invoke(ex.Message);
                return false;
            }
        }
